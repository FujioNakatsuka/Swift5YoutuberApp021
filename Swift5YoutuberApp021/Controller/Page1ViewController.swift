...
//  Swift5YoutuberApp021
//
//  Created by 中塚富士雄 on 2020/09/10.
...
//⭐️SubclassはUIViewControllerだったがtableViewにできるのか❓
    ...
    //        cell.imageView?.sd_setImage(with: profileImageURL, completed: nil)
        ...
        func getData(){
            //リクエストを送る。JSON解析を行う。値が返ってくるので、for文で全て配列に入れる
             var text =  "https://www.googleapis.com/youtube/v3/search?key=AIzaSyAvnVD83fgBV6AB5MjsbtrVMEAScBF_fS8&q=ナラティブブック&part=snippet&maxResults=19&order=date"
            ...
            AF.request(url, method: .get, parameters: nil, encoding: JSONEncoding.default) .responseJSON{(responce) in
                ...
                case .success:
                    ...
                    for i in 0...19{
                        ...
                        //let title = json["items"][i]["snippet"]["publishedAt"].string
